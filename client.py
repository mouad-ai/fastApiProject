...
def rc4(key, data):
    ...
    return result
# Encrypt the message using RC4 and the shared secret
# Encrypt the message using RC4 and the shared secret
...
def encrypt_message(message, shared_secret):
    ...
    encrypted_data = rc4(shared_secret.to_bytes(16, 'big'), encoded_message)
    encrypted_hex = encrypted_data.hex()
    return encrypted_hex

# Send the encrypted message to the server for decryption
def send_encrypted_message(encrypted_message, server_public_key):
    response = requests.post('http://localhost:5000/decrypt_message', json={'encrypted_message': encrypted_message, 'client_public_key': client_public_key})
    decrypted_message = response.json()['response']
...
# Retrieve the server's public key
server_public_key = retrieve_server_public_key()

# Generate shared secret using the client's private key and the server's public key
...
# Send the encrypted message to the server for decryption and receive the decrypted message
decrypted_message = send_encrypted_message(encrypted_message, server_public_key)

# Print the decrypted message received from the server
print(f"Response Message: {decrypted_message}")
